...
const levelFieldMap: Record<string, { percentage: string; currency: string; money: string; notes: string }> = {
  'Senior Level': {
...
const levelToKey = (level: string): keyof LevelBasedHiring => {
  switch (level) {
    case 'Senior Level': return 'seniorLevel';
    case 'Executives': return 'executives';
    case 'Non-Executives': return 'nonExecutives';
...
const StandardContractForm = ({ formData, setFormData }: StandardContractFormProps) => {
  ...
  const handleLevelChangeFix = (level: string) => {
    ...
    clonedFormData.levelBasedHiring.levelTypes = clonedFormData.levelBasedHiring.levelTypes.includes(level) ? clonedFormData.levelBasedHiring.levelTypes.filter(l => l !== level) : [...clonedFormData.levelBasedHiring.levelTypes, level]
  ...
  const handleLevelChangeAdvance = (level: string) => {
    const clonedFormData = structuredClone(formData);
    clonedFormData.levelBasedAdvanceHiring.levelTypes = clonedFormData.levelBasedAdvanceHiring.levelTypes.includes(level) ? clonedFormData.levelBasedAdvanceHiring.levelTypes.filter(l => l !== level) : [...clonedFormData.levelBasedAdvanceHiring.levelTypes, level]
    setFormData(clonedFormData);
    if (activeLevel === level) setActiveLevel(null);
  };

  const handleValueChangeAdvance = (level: string, value: number | string, type: keyof LevelValueAdvance) => {
    const clonedFormData = structuredClone(formData);
    if (type === 'percentage' && typeof value === 'number') {
      (clonedFormData.levelBasedAdvanceHiring[levelToKey(level)] as LevelValueAdvance).percentage = value;
    } else if (type === 'notes' && typeof value === 'string') {
      (clonedFormData.levelBasedAdvanceHiring[levelToKey(level)] as LevelValueAdvance).notes = value;
    } else if (type === 'currency' && typeof value === 'string') {
      (clonedFormData.levelBasedAdvanceHiring[levelToKey(level)] as LevelValueAdvance).currency = value;
    } else if (type === 'amount' && typeof value === 'number') {
      (clonedFormData.levelBasedAdvanceHiring[levelToKey(level)] as LevelValueAdvance).amount = value;
    }
    setFormData(clonedFormData);
  };
        ...
        <div className="flex flex-row gap-4">
          ...
          <div className="flex-1 space-y-1">
            ...
            <div className="grid gap-2">
              <DatePicker
                ...
                setValue={(value: Date) => setFormData((prev: typeof formData) => ({ ...prev, contractStartDate: value }))}
              />
            </div>
          </div>
          {/* Contract End Date */}
          <div className="flex-1 space-y-1">
            ...
            <div className="grid gap-2">
              <DatePicker
                ...
                setValue={(value: Date) => setFormData((prev: typeof formData) => ({ ...prev, contractEndDate: value }))}
              />
            </div>
          </div>
          {/* Contract Type */}
          <div className="flex-1 space-y-1">
            ...
            <Select
              ...
              onValueChange={value => setFormData((prev: typeof formData) => ({ ...prev, contractType: value }))}
            >
              <SelectTrigger>
              ...
              <div className="border-2 shadow-sm rounded-lg p-2 cursor-pointer transition-colors w-full border-primary bg-primary/5">
                <div className="flex flex-col sm:flex-row items-center sm:space-x-4 space-y-2 sm:space-y-0">
                  ...
                  <PercentageCurrencyNotesRow
                    ...
                    onPercentageChange={value => setFormData((prev: typeof formData) => ({ ...prev, fixedPercentage: value }))}
                    showCurrency
                    currencyValue={formData.advanceMoneyCurrency}
                    onCurrencyChange={value => setFormData((prev: typeof formData) => ({ ...prev, advanceMoneyCurrency: value }))}
                    amountValue={formData.advanceMoneyAmount}
                    onAmountChange={value => setFormData((prev: typeof formData) => ({ ...prev, advanceMoneyAmount: value }))}
                    notesValue={formData.fixedPercentageAdvanceNotes}
                    onNotesChange={value => setFormData((prev: typeof formData) => ({ ...prev, fixedPercentageAdvanceNotes: value }))}
                  />
                </div>
              </div>
              ...
              <div className="border-2 shadow-sm rounded-lg p-2 cursor-pointer transition-colors w-full border-primary bg-primary/5">
                <div className="flex flex-col sm:flex-row items-center sm:space-x-4 space-y-2 sm:space-y-0">
                  ...
                  <PercentageCurrencyNotesRow
                    ...
                    onPercentageChange={value => setFormData((prev: typeof formData) => ({ ...prev, fixWithoutAdvanceValue: value }))}
                    notesValue={formData.fixWithoutAdvanceNotes}
                    onNotesChange={value => setFormData((prev: typeof formData) => ({ ...prev, fixWithoutAdvanceNotes: value }))}
                  />
                </div>
              </div>
              <ContractDocumentUpload
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row items-center sm:space-x-4 space-y-2 sm:space-y-0">
                          ...
                          <PercentageCurrencyNotesRow
                            percentageValue={(formData.levelBasedHiring[levelToKey(level)] as LevelValue).percentage}
                            onPercentageChange={value => handleValueChangeFix(level, value, "percentage")}
                            notesValue={(formData.levelBasedHiring[levelToKey(level)] as LevelValue).notes}
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row items-center sm:space-x-4 space-y-2 sm:space-y-0">
                          <h4 className="font-medium text-xs sm:text-sm w-28">{level}</h4>
                          <PercentageCurrencyNotesRow
                            percentageValue={(formData.levelBasedAdvanceHiring[levelToKey(level)] as LevelValueAdvance).percentage}
                            onPercentageChange={value => handleValueChangeAdvance(level, value, "percentage")}
                            showCurrency={formData.contractType === 'Level Based With Advance'}
                            currencyValue={(formData.levelBasedAdvanceHiring[levelToKey(level)] as LevelValueAdvance).currency}
                            onCurrencyChange={value => handleValueChangeAdvance(level, value, "currency")}
                            amountValue={(formData.levelBasedAdvanceHiring[levelToKey(level)] as LevelValueAdvance).amount}
                            onAmountChange={value => handleValueChangeAdvance(level, value, "amount")}
                            notesValue={(formData.levelBasedAdvanceHiring[levelToKey(level)] as LevelValueAdvance).notes}

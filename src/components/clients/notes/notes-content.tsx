...
export function NotesContent() {
  ...
  // Extract client id from the route, e.g. /clients/[id]
  // In Next.js app router, params are available via props or useParams, but here we can use window.location or ask user to pass as prop if not available.
  // We'll try to extract from URL as fallback:
  let clientId = "";
  if (typeof window !== "undefined") {
    const match = window.location.pathname.match(/clients\/(\w+)/);
...
useEffect(() => {
 ...
 const handleUpdateNote = async (updated: { content: string }) => {
    ...
    try {
      const res = await axios.put(`${API_BASE}/${editNote.id}`, {
